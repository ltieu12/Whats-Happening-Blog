...
<head>
  ...
  <title>What's Happening</title> <!-- Change the webpage title -->
            ...
            <div class="swiper sliderFeaturedPosts">
              <div class="swiper-wrapper">
                <div class="swiper-slide">
                  <a href="single-post.html" class="img-bg d-flex align-items-end" style="background-image: url('assets/img/post-slide-1.jpg');">
                    <div class="img-bg-inner">
                      <h2>What's Happening in your Community</h2> <!-- Change header title of each slide -->
                ...
                <?php
                  ...
                  // retrieve events from the database, order from the most current SubmitDate to the oldest
                  $latestQuery = "SELECT EventID, EventTitle, EventDesc FROM Events ORDER BY SubmitDate DESC";
                  $result1 = mysqli_query($conn, $latestQuery);
                  if ($result1->num_rows > 0) {
                    // get the first data from the query, which is the event that is most currently submitted
                    // get the required data to display in the slider
                    ...
                    $section = <<<HTML
                      <div class="swiper-slide">
                        <a href="single-post.php?eventNumber=$eventID" class="img-bg d-flex align-items-end" style="background-image: url('assets/img/post-slide-2.jpg');">
                          <div class="img-bg-inner">
                            <h2>Latest Added Event</h2> <!-- Change header title of each slide -->
                            <p><span><strong>$title: </strong></span>$description</p>
                          </div>
                        </a>
                      </div>
                    HTML;
                  ...
                  // retrieve events from the database, order from the oldest EventDate to the newest
                  $soonQuery = "SELECT EventID, EventTitle, EventDesc FROM Events ORDER BY EventDate";
                  $result2 = mysqli_query($conn, $soonQuery);
                  if ($result2->num_rows > 0) {
                    // get the first data from the query, which is the event that is closest to today (assume all the events are happening in the future)
                    // get the required data to display in the slider
                    ...
                    $section = <<<HTML
                      <div class="swiper-slide">
                        <a href="single-post.php?eventNumber=$eventID" class="img-bg d-flex align-items-end" style="background-image: url('assets/img/post-slide-3.jpg');">
                          <div class="img-bg-inner">
                            <h2>Happening Soon</h2> <!-- Change header title of each slide -->
                            <p><span><strong>$title: </strong></span>$description</p>
                          </div>
                        </a>
                      </div>
