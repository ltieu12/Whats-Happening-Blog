<?php
  // if user did not login, direct them to the login page to login. Only logged in account can post new event
  session_start();
...
<head>
  ...
  <title>What's Happening - Post Event</title> <!-- Change the webpage title -->
      ...
      <div class="container" data-aos="fade-up">
        ...
        <div class="row">
          <div class="col-lg-12 text-center mb-5">
            <h1 class="page-title">Post New Event</h1> <!-- Change the title to "Post New Event" -->
          </div>
        </div>
        ...
        <div class="form mt-5">
          <?php
            ...
            // get the groupID from session array (already assign when user logged in)
            $groupID = $_SESSION['GroupID'];
            // get the group information from account's groupID
            $groupQuery = "SELECT GroupName FROM Groups WHERE GroupID = '$groupID'";
            $groupResult = mysqli_query($conn, $groupQuery);
            if ($groupResult->num_rows > 0) {
              $row = $groupResult->fetch_assoc();
              $groupName = $row['GroupName'];
            }
            // display the group name on top of the form to indicate which group is posting new event
            if (isset($groupName)) {
              $section = <<<HTML
              <div class="text-center">
              <p class="pt-4"><strong>$groupName</strong></p>
              </div>
            HTML;
            echo $section;
            }
          ...
          <form action="post.php" method="post" role="form">
            <!-- Change the title for each field required for a new event -->
            <!-- using Bootstrap style to re-style the form so it has the same feeling as the original form. Source: https://getbootstrap.com/ -->
            ...
            <div class="form-group py-1">
              <input type="text" class="form-control" name="time" id="time" placeholder="Your Event Time (Format: 12-hour format with AM/PM)" required>
            </div>
            <div class="form-group py-1">
            ...
            <div class="text-center py-1"><button class="px-4 py-2 bg-dark text-white" type="submit">Submit</button></div> <!-- Change the button to "Login" -->
          </form>
        </div><!-- End Contact Form -->
        <?php
          // assign each field value from the form to associate variables
          $title;
          $date;
          $time;
          $type;
          ...
          if (isset($title) && isset($date) && isset($type) && isset($image) && isset($description)) {
            // format date/time before submitting to database
            $dateTime = $date." ".$time;
            $eventDate = date("Y-m-d H:i:s", strtotime($dateTime));
            $submitDate = date("Y-m-d H:i:s");

            // get the eventTypeID from the input event type
            $eventTypeQuery = "SELECT EventTypeID FROM EventTypes WHERE EventType = '$type'";
            $eventTypeResult = mysqli_query($conn, $eventTypeQuery);
            if ($groupResult->num_rows > 0) {
              $row = $eventTypeResult->fetch_assoc();
              $eventTypeID = (int) $row['EventTypeID'];
            }
            
            // use prepare statement to prepare query and bind associated data to add to the database
            if (isset($eventTypeID) && isset($groupID)) {
              $statement = $conn->prepare("INSERT INTO Events (EventTypeID, GroupID, EventDate, SubmitDate, EventTitle, EventImage, EventDesc) VALUES (?, ?, ?, ?, ?, ?, ?)");
              $statement->bind_param("iisssss", $eventTypeID, $groupID, $eventDate, $submitDate, $title, $image, $description);
              $statement->execute();
